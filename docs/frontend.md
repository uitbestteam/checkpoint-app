...
- **Check-in 2 kiểu** (content tách khỏi Sheet): `CreateCheckpointForm` + `LocationPermissionContent` render trong **Sheet** (nút Map, qua `useCheckin`) hoặc **full-page** (tab Create → success tự về Map). Khi có hành trình đang active, form hiện **dropdown chọn hành trình** (🧳 "Ghi vào hành trình") — liệt kê mọi journey active + "Không gắn hành trình", **mặc định chọn active mới nhất** (sort theo `updated_at`), gửi `journey_id` đã chọn; BE validate quyền sở hữu.
- **Upload ảnh thẳng vào R2** (`lib/api.ts` `uploadCheckpointImages`): thay vì multipart FE→BE→R2, giờ dùng 3 bước — (1) `POST /checkpoints/{id}/upload-urls` lấy presigned PUT URLs từ BE; (2) `fetch(presignedUrl, PUT, file)` PUT thẳng R2 bằng native fetch (không qua axios, không cần Authorization header); (3) `POST /checkpoints/{id}/images/link` gửi keys cho BE ghi DB. **Prerequisite:** R2 CORS phải allow PUT từ FE origin.
- **Geocode phía client** (`lib/photon.ts` + `hooks/useGeocode.ts`): gọi `photon.komoot.io/reverse` ngay khi form mở, chạy song song với user điền form. `useGeocode(lat, lng)` — module-level cache `Map<key, GeoResult>` (key làm tròn 4dp ≈ 11m) → mở form lần 2 cùng tọa độ trả ngay, không re-fetch. Address hint hiện dưới ô tên; tên địa điểm **tự điền** từ `address.split(",")[0]` khi là new place và name chưa có. Gửi `address` + `province` trong payload; BE không gọi Photon nữa.
- **Camera preview-after-shot** (`CameraCaptureSheet`): chụp → preview toàn màn hình → "Chụp lại" (pop shot cuối, về viewfinder) / "Tiếp tục" (giữ shots, về viewfinder, ẩn nếu đủ `maxCount`) / "Xong" (trả files). Counter `n/maxCount` ở top bar; "Xong" cũng hiện ở top bar khi đang ở viewfinder và đã có shot. Webcam giữ mounted khi ở preview (tránh khởi tạo lại). Gallery picks thêm vào `shots[]`, không cần preview riêng. `maxCount` prop (optional, mặc định 3); `CreateCheckpointForm` truyền `MAX_IMAGES - picked.length`.
- **Check-in từ ảnh** (`CreatePage` tab "Từ ảnh" + `PhotoCheckinTab`): tab mới bên cạnh "Cắm cờ" — user chọn 1 ảnh từ thư viện (gallery only, không capture — ảnh thư viện mới có EXIF GPS), `extractGPS` (exifr `gps()`) đọc tọa độ từ EXIF; nếu không có GPS → hiện lỗi + nút "Thử ảnh khác"; nếu có GPS → render `CreateCheckpointForm` với `at` = tọa độ EXIF + `initialFiles=[file]` (ảnh điền sẵn). `CreateCheckpointForm` nhận `initialFiles?: File[]`: `useEffect` mount-only xử lý qua `addFileArray`; cờ `initialFiles?.length` tắt auto-open camera để không phá luồng photo-checkin. Dùng `exifr@7` (hỗ trợ JPEG/HEIC/WebP, async, nhẹ).
...
- **Share card (#9 + J5)** — **Canvas2D** (`lib/shareCard.ts`): vẽ card→PNG bằng `<canvas>` (`drawImage` ảnh object-cover + `fillText` wrap + gradient + font Montserrat qua `document.fonts.load`). **Lý do bỏ html-to-image**: nó dùng SVG `<foreignObject>` → **WebKit/iOS render `<img>` bên trong ra TRẮNG** (desktop OK, iPhone mất ảnh). Canvas2D chạy mọi nền tảng. `renderCheckpointCard`/`renderJourneyCard`/`renderProfileCard` (model "block" stack: photos/header/quote/stats/footer). `ShareButton` nhận `render: () => Promise<Blob>` → `shareImageBlob` (Web Share file + `url` → fallback download → text).
  ...
  - **Deep link**: Web Share **`url`** sử dụng Backend share URL (`${API_URL}/share/checkpoint/{id}` · `/share/journey/{id}` · `/share/user/{username}`) để tối ưu hóa SEO hiển thị Open Graph preview cho Crawler (được redirect tự động về frontend cho người dùng thường). Route `/c/:id` `/j/:id` `/u/:username` mở các sheet tương ứng; URL sống qua màn login. `JourneyLink` lazy.

### Journey (Phase 3 ✅)
- **JourneysSection** (trong Profile): list hành trình + "+ Bắt đầu" (luôn hiện, tạo được nhiều journey active). **Nhiều journey active cùng lúc** — mỗi cái 1 banner xanh "Đang đi" + nút "Kết thúc" riêng (`updateJourney(id, {is_active})`). Tạo journey → tự set active → trở thành mặc định trong dropdown check-in.
