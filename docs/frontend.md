...
- **Camera gốc khi check-in**: trên thiết bị touch, khu vực ảnh có 2 action riêng: **Chụp ảnh** dùng input `accept="image/*" capture="environment"` để yêu cầu mở camera app gốc (Android tự xử lý lens/HDR/chống méo), **Thư viện** dùng input không `capture` và hỗ trợ chọn nhiều ảnh. Desktop chỉ hiện **Chọn ảnh**. Camera trả một ảnh mỗi lần; bấm lại để chụp tiếp tới tối đa 3 ảnh. Browser/OS vẫn có quyền hiện chooser thay vì mở thẳng camera. Tất cả ảnh đều qua `prepareImage` (HEIC → JPEG + downscale).
