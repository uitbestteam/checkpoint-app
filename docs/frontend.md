...
- **Share card (#9 + J5)**: `lib/share.ts` snapshot DOM node → PNG (`html-to-image`, **lazy-import**, `skipFonts`) → **Web Share API** → fallback download → text. `ShareButton` + 3 card (`share/cards.tsx`): checkpoint (CheckpointDetailSheet), hành trình (JourneyDetailSheet), hộ chiếu (ProfilePage). Card render off-screen brand-only (tránh CORS taint khi render).
  - **Ảnh nhúng vào card**: card render kèm **ảnh thật** (`PhotoGrid` tới 3 ảnh: 1=hero, 2=đôi, 3=1 lớn+2 nhỏ; `crossOrigin="anonymous"`) → share **đúng 1 ảnh PNG** chứa cả ảnh. `toPng` dùng `imagePlaceholder` (1×1 trong suốt) → ảnh lỗi/CORS bị thay placeholder, card **vẫn render** (không vỡ). **Cần R2 CORS** để inline ảnh remote.
  - **Deep link**: vẽ link vào card + truyền qua Web Share **`url`** (giữ tốt hơn `text` khi share kèm file). `${window.location.origin}/c/{id}` · `/j/{id}` (origin tự lấy, không env). Route `/c/:id` `/j/:id` (`pages/LinkRoutes.tsx`) mở detail sheet; URL sống qua màn login (react-router). `JourneyLink` lazy (maplibre không vào main bundle).
