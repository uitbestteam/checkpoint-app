...
- **Camera in-app** (`CameraCaptureSheet`, lib `react-webcam`): trên **mobile + secure context + có camera** (`canUseCamera()`), nút thêm ảnh mở camera full-screen (preview · Photos góc trái mở gallery · Shutter giữa · Đổi camera/lens góc phải); sau khi cấp quyền, app liệt kê `videoinput`, cho cycle bằng `deviceId`, hiện label camera và nhớ camera đã chọn để tránh Android Chrome tự mở ultra-wide. Chụp liên tiếp tới 3 ảnh → Xong. Device đã lưu lỗi → thử lại camera sau mặc định; `getUserMedia` vẫn lỗi → fallback gallery. Desktop/không hỗ trợ → file picker thường. Ảnh chụp + chọn đều qua `prepareImage` (downscale). *(Lưu ý: nút trong sheet đều `type="button"` vì render trong `<form>`.)*
