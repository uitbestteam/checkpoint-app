...
- **Camera in-app** (`CameraCaptureSheet`, lib `react-webcam`): trên **mobile + secure context + có camera** (`canUseCamera()`), nút thêm ảnh mở camera full-screen (preview · Photos góc trái mở gallery · Shutter giữa · Lật trước/sau góc phải); chụp liên tiếp tới 3 ảnh → Xong. Desktop/không hỗ trợ → file picker thường. `getUserMedia` lỗi (iOS PWA <16, từ chối quyền) → tự fallback gallery. Ảnh chụp + chọn đều qua `prepareImage` (downscale). *(Lưu ý: nút trong sheet đều `type="button"` vì render trong `<form>`.)*
...
- **Share card (#9 + J5)**: `lib/share.ts` snapshot DOM node → PNG (`html-to-image`, **lazy-import**, `skipFonts`) → **Web Share API** → fallback download → text. `ShareButton` + 3 card (`share/cards.tsx`): checkpoint (CheckpointDetailSheet), hành trình (JourneyDetailSheet), hộ chiếu (ProfilePage). Card render off-screen brand-only (tránh CORS taint khi render).
  - **Ảnh nhúng vào card**: `useCardImages` **fetch ảnh → data URL** (`mode:cors` + query `?cors=1` để né bản đã cache không-CORS từ `<img>` thường — r2.dev có trả CORS đúng, nhưng `<img>` trước đó cache bản không-Origin) rồi `PhotoGrid` render (tới 3 ảnh: 1=hero, 2=đôi, 3=1 lớn+2 nhỏ). Data URL nên html-to-image bake vào **đúng 1 ảnh PNG**, không taint. **Cần R2 CORS** cho app origin; ảnh fail → bỏ qua (card gọn, không khối trống). Link **KHÔNG vẽ vào ảnh**.
  - **Deep link**: truyền qua Web Share **`url`** (`${window.location.origin}/c/{id}` · `/j/{id}`, origin tự lấy). Route `/c/:id` `/j/:id` (`pages/LinkRoutes.tsx`) mở detail sheet; URL sống qua màn login. `JourneyLink` lazy (maplibre không vào main bundle).
