...
- **Camera single-shot** (`CameraCaptureSheet`): bỏ multi-shot flow (không còn `shots[]` state, counter, nút "Xong"). Chụp 1 cái → `onDone([file])` + `onClose()` ngay. Chọn gallery cũng trả về 1 file ngay. Bỏ `maxCount` prop.
