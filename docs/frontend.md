...
- **Share card (#9 + J5)**: `lib/share.ts` snapshot DOM node → PNG (`html-to-image`, **lazy-import**, `skipFonts`) → **Web Share API** → fallback download → text. `ShareButton` + 3 card (`share/cards.tsx`): checkpoint (CheckpointDetailSheet), hành trình (JourneyDetailSheet), hộ chiếu (ProfilePage). Card render off-screen brand-only (tránh CORS taint khi render).
  - **Ảnh nhúng vào card**: `useCardImages` **fetch ảnh → data URL** (`mode:cors`, `cache:reload` để né bản đã cache không-CORS từ `<img>` thường) rồi `PhotoGrid` render (tới 3 ảnh: 1=hero, 2=đôi, 3=1 lớn+2 nhỏ). Data URL nên html-to-image bake vào **đúng 1 ảnh PNG**, không taint. **Cần R2 CORS** cho app origin; ảnh fail → bỏ qua (card gọn, không khối trống). Link **KHÔNG vẽ vào ảnh**.
