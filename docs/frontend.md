...
- **Geocode phía client** (`lib/photon.ts`): gọi `photon.komoot.io/reverse` ngay khi form mở (chạy song song với user điền form), parse `address` + `province` từ GeoJSON properties, hiện địa chỉ nhỏ dưới ô tên địa điểm, gửi cả hai trong payload check-in. BE không còn gọi Photon nữa.
