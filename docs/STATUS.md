...
- **Mới (2026-06-14):** **Google Places API gợi ý check-in** — `GET /places/suggest?lat&lng` (backend proxy, `internal/googleplaces`): gọi Google Places Nearby Search, cache in-memory 1h theo rounded coords (111m grid), map 14 Google types → 5 category. FE (`CreateCheckpointForm`) chạy `useQuery` song song với Photon: nếu suggestion về thì auto-fill `name` + `category`, hiện chip "Gợi ý từ Google Maps · Xoá"; khi API key trống hoặc quota hết thì fallback về Photon như cũ. Key: `GOOGLE_PLACES_API_KEY` (optional env).
