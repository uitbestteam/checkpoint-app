...
- **Video transcode server-side** (`POST /checkpoints/video`, multipart `video` + optional `thumbnail`): nhiều browser không xuất được MP4 nên FE gửi video gốc (mọi định dạng) lên đây; BE chạy **ffmpeg** (`internal/checkpoint/transcode.go`) transcode → MP4 (H.264 baseline, longest edge ≤720, `+faststart`, AAC, `-t 10`, auto-rotate), upload thẳng R2 dưới `checkpoints/<rand>/...mp4`, trả `{ object_key, thumbnail_key, type:"video" }` để FE gắn vào `POST /checkpoints`. Validate size authoritative: input ≤ 60MB (`MaxBytesReader` + `maxVideoInputBytes`), output ≤ 12MB (`maxVideoOutputBytes`). Rate-limit dùng `uploadSlots` limiter. Cần ffmpeg trong image runtime → Dockerfile đổi `distroless/static` → `alpine` + `apk add ffmpeg`. `ErrTranscodeUnavailable` → 503 nếu ffmpeg vắng mặt.
